...
public class Plot19 : MonoBehaviour
{
    ...
    void Update()
    {
        if((Plant19.isDead == 1) && (Plant19.isThere == 0))
        {
            GetComponent<SpriteRenderer>().sprite = noWater;
            Plant19.isDead = 0;
            isWatered = 0;
    ...
    void OnMouseOver()
    {
        if((Seed1.seedTimer > 0) && (Seed1.seedDown == 1) && (Plant19.isThere == 0))
        {
            if(Seed1.seedType == 1)
            {
    ...
    void OnMouseDown()
    {
        if((Plant19.isThere == 1) && (Plant19.isReady == 1))
        {
