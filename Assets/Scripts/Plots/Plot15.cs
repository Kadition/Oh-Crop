...
public class Plot15 : MonoBehaviour
{
    ...
    void Update()
    {
        if((Plant15.isDead == 1) && (Plant15.isThere == 0))
        {
            GetComponent<SpriteRenderer>().sprite = noWater;
            Plant15.isDead = 0;
            isWatered = 0;
    ...
    void OnMouseOver()
    {
        if((Seed1.seedTimer > 0) && (Seed1.seedDown == 1) && (Plant15.isThere == 0))
        {
            if(Seed1.seedType == 1)
            {
    ...
    void OnMouseDown()
    {
        if((Plant15.isThere == 1) && (Plant15.isReady == 1))
        {
