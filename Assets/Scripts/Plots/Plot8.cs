...
public class Plot8 : MonoBehaviour
{
    ...
    void Update()
    {
        if((Plant8.isDead == 1) && (Plant8.isThere == 0))
        {
            GetComponent<SpriteRenderer>().sprite = noWater;
            Plant8.isDead = 0;
            isWatered = 0;
    ...
    void OnMouseOver()
    {
        if((Seed1.seedTimer > 0) && (Seed1.seedDown == 1) && (Plant8.isThere == 0))
        {
            if(Seed1.seedType == 1)
            {
    ...
    void OnMouseDown()
    {
        if((Plant8.isThere == 1) && (Plant8.isReady == 1))
        {
            killPlant = 1;
            isWatered = 0;
            GetComponent<SpriteRenderer>().sprite = noWater;
