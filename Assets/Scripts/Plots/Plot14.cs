...
public class Plot14 : MonoBehaviour
{
    ...
    void Update()
    {
        if((Plant14.isDead == 1) && (Plant14.isThere == 0))
        {
            GetComponent<SpriteRenderer>().sprite = noWater;
            Plant14.isDead = 0;
            isWatered = 0;
    ...
    void OnMouseOver()
    {
        if((Seed1.seedTimer > 0) && (Seed1.seedDown == 1) && (Plant14.isThere == 0))
        {
            if(Seed1.seedType == 1)
            {
    ...
    void OnMouseDown()
    {
        if((Plant14.isThere == 1) && (Plant14.isReady == 1))
        {
