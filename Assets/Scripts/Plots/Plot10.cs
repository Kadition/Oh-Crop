...
public class Plot10 : MonoBehaviour
{
    ...
    void Update()
    {
        if((Plant10.isDead == 1) && (Plant10.isThere == 0))
        {
            GetComponent<SpriteRenderer>().sprite = noWater;
            Plant10.isDead = 0;
            isWatered = 0;
    ...
    void OnMouseOver()
    {
        if((Seed1.seedTimer > 0) && (Seed1.seedDown == 1) && (Plant10.isThere == 0))
        {
            if(Seed1.seedType == 1)
            {
    ...
    void OnMouseDown()
    {
        if((Plant10.isThere == 1) && (Plant10.isReady == 1))
        {
            killPlant = 1;
            isWatered = 0;
            GetComponent<SpriteRenderer>().sprite = noWater;
