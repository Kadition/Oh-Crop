...
public class Plot5 : MonoBehaviour
{
    ...
    void Update()
    {
        if((Plant5.isDead == 1) && (Plant5.isThere == 0))
        {
            GetComponent<SpriteRenderer>().sprite = noWater;
            Plant5.isDead = 0;
            isWatered = 0;
    ...
    void OnMouseOver()
    {
        if((Seed1.seedTimer > 0) && (Seed1.seedDown == 1) && (Plant5.isThere == 0))
        {
            if(Seed1.seedType == 1)
            {
    ...
    void OnMouseDown()
    {
        if((Plant5.isThere == 1) && (Plant5.isReady == 1))
        {
