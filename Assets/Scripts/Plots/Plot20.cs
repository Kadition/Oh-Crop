...
public class Plot20 : MonoBehaviour
{
    ...
    void Update()
    {
        if((Plant20.isDead == 1) && (Plant20.isThere == 0))
        {
            GetComponent<SpriteRenderer>().sprite = noWater;
            Plant20.isDead = 0;
            isWatered = 0;
    ...
    void OnMouseOver()
    {
        if((Seed1.seedTimer > 0) && (Seed1.seedDown == 1) && (Plant20.isThere == 0))
        {
            if(Seed1.seedType == 1)
            {
    ...
    void OnMouseDown()
    {
        if((Plant20.isThere == 1) && (Plant20.isReady == 1))
        {
