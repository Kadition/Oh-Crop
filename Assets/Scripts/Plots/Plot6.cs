...
public class Plot6 : MonoBehaviour
{
    ...
    void Update()
    {
        if((Plant6.isDead == 1) && (Plant6.isThere == 0))
        {
            GetComponent<SpriteRenderer>().sprite = noWater;
            Plant6.isDead = 0;
            isWatered = 0;
    ...
    void OnMouseOver()
    {
        if((Seed1.seedTimer > 0) && (Seed1.seedDown == 1) && (Plant6.isThere == 0))
        {
            if(Seed1.seedType == 1)
            {
    ...
    void OnMouseDown()
    {
        if((Plant6.isThere == 1) && (Plant6.isReady == 1))
        {
