...
public class Plot23 : MonoBehaviour
{
    ...
    void Update()
    {
        if((Plant23.isDead == 1) && (Plant23.isThere == 0))
        {
            GetComponent<SpriteRenderer>().sprite = noWater;
            Plant23.isDead = 0;
            isWatered = 0;
    ...
    void OnMouseOver()
    {
        if((Seed1.seedTimer > 0) && (Seed1.seedDown == 1) && (Plant23.isThere == 0))
        {
            if(Seed1.seedType == 1)
            {
    ...
    void OnMouseDown()
    {
        if((Plant23.isThere == 1) && (Plant23.isReady == 1))
        {
